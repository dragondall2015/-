...
class Fighter(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.image = pygame.image.load('탈모빔.png')
...
class Missile(pygame.sprite.Sprite):
    def __init__(self, xpos , ypos, speed):
        ...
        self.image = pygame.image.load('탈모약.PNG')
        self.rect = self.image.get_rect()
        self.rect.x = xpos
        self.rect.y = ypos
        self.speed = speed
    ...
    def collibe(self, sprites):
        for sprite in sprites:
            if pygame.sprite.collide_rect(self, sprite):
                return sprite


class hair_loss(pygame.sprite.Sprite):
    def __init__(self, xpos, ypos, speed):
        super(hair_loss, self).__init__()
        rock_images = ('탈모빔.png', '담배 1.png', '담배2.png', '맥주.png', '맥주2.png', '소주.png', '소주2.png',\
                       '양주.png', '양주2.PNG', '햄버거.png', '햄버거2.png', '치킨.png', '치킨2.png')
...
def game_loop():
    ...
    background_image = pygame.image.load('noname01.bmp')
    gameover_sound = pygame.mixer.Sound('gameover.wav')
    pygame.mixer.music.load('탈모르파티.wav')
    ...
    while not done:
        ...
        if random.randint(1, occur_prob) == 1:
            for i in range(occur_of_rocks):
                ...
                rock = hair_loss(random.randint(0, WINDOW_WIDTH - 30), 0, speed)
                rocks.add(rock)

        draw_text('파괴한 탈모음식: {}'.format(shot_count), default_font, screen, 100, 20, RED)
        draw_text('파괴된 모발: {}'.format(count_missed), default_font, screen, 400, 20, RED)

        for missile in missiles:
            rock = missile.collibe(rocks)
            if rock:
                missile.kill()
                rock.kill()
...
def game_menu():
    start_image = pygame.image.load('noname01.bmp')
    ...
    draw_text('모발를 지켜라!', font_70, screen, draw_x, draw_y, YELLOW)
    draw_text('엔터 키를 누르면', font_40, screen, draw_x, draw_y + 200, BLACK)
    draw_text('게임이 시작됩니다.', font_40, screen, draw_x, draw_y + 250, BLACK)
